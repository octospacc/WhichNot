...
import { AppContext, ModalHeader } from "./app";
import { STRINGS } from "./SystemData";
...
export function AppSettingsModal() {
  ...
  const AestheticsSettings = () => <>
    <h4>{STRINGS.get('Aesthetics')}</h4>

    <p><label>{STRINGS.get('Color Scheme')}: <select value={state.prefs.colorScheme} onChange={ev => setState(s => ({ ...s, prefs: { ...s.prefs, colorScheme: ev.target.
// @ts-ignore
    value } }))}>
      <option value="system">Auto ({STRINGS.get('System')})</option>
      <option value="light">☀️ {STRINGS.get('Light')}</option>
      <option value="dark">🌑 {STRINGS.get('Dark')}</option>
    </select></label></p>

    <p>
      <label>{STRINGS.get('Message Input Font')}:
      </label> <select value={state.prefs.messageInput?.fontFamily} onChange={ev => setState(s => ({ ...s, prefs: { ...s.prefs, messageInput: { ...s.prefs.messageInput, fontFamily: ev.target.
        // @ts-ignore
        value } } }))}>
        <option value="">{STRINGS.get('Default')} (Browser)</option>
    ...
    <p><label>{STRINGS.get('Language')}: <select value={state.prefs.language} onChange={ev => setState(s => ({ ...s, prefs: { ...s.prefs, language: ev.target.
// @ts-ignore
    value }}))}>
      <option value="">Auto ({STRINGS.get('System')})</option>
      <option value="en">🇬🇧 English</option>
      <option value="it">🇮🇹 Italiano</option>
    </select></label></p>
  </>;

  return <div class="AppSettingsModal" tabindex={-1}>
    ...
    <select onChange={ev => setExportTxt(ev.target.
// @ts-ignore
    value ? exportDataNotebook(getNotebook(ev.target.
// @ts-ignore
    value)) : exportData())}>
      <option value="">{STRINGS.get('Full')}</option>
      {state.notebooks.map(notebook => <option value={notebook.id}>{notebook.emoji} {notebook.name}</option>)}
    </select>
    <textarea readOnly rows={8} onFocus={ev => ev.target.
// @ts-ignore
    setSelectionRange(0, -1)}>{exportTxt}</textarea>

    <h4>{STRINGS.get('Import Data')}</h4>
    <textarea rows={8} placeholder={STRINGS.get('Paste JSON')} onInput={ev => setImportTxt(ev.target.
// @ts-ignore
    value)} />
    <label><input type="checkbox" checked={importReset} onChange={ev => setImportReset(ev.target.
// @ts-ignore
    checked)} />{STRINGS.get('Reset Data on Import')}</label>
    <span> </span>
